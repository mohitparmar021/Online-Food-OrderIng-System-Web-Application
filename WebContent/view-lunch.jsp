				...
				<table border="2px"
					...
					<tbody>
						...
						<tr style="color: dark; font-size: 20px">
							...
							<td><a
								href="update-lunch-menu.jsp?item-id=<%=rs.getInt(1)%>&name=<%=rs.getString(2) %>&price=<%=rs.getString(3) %>&time=<%=rs.getString(4) %>"
								class="btn-lg bg-warning text-white pl-3 pd-2">Update</a></td> 
							<td><a
								href="delete-lunch-menu.jsp?item-id=<%=rs.getString(1)%>"
								class="btn-lg bg-warning text-white pl-3 pd-2">Delete</a></td>
